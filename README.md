...
- **免费版**：全部功能不受限（关键词数量 / ASIN 数量 / 扫描页数都不限），但**总共只能查询 3 次**，用完需激活 Pro
- **Pro 版**：解锁无限次查询；一次性买断，激活码解锁
  - 国际用户：海外平台（Lemon Squeezy/Payhip），付款后自动发码
  - 国内用户：淘宝虚拟商品自动发货（微信/支付宝付款后自动发卡密）
  - 扩展按界面语言自动显示对应购买入口；**具体价格以商品页实际标价为准**（扩展内不写死价格，避免改价后忘记同步）
- 收费系统配置说明见 [docs/payment-setup.md](docs/payment-setup.md)
- 激活码生成/验证工具：[tools/keygen.mjs](tools/keygen.mjs)（零依赖，Ed25519 签名，私钥只在您本机）
